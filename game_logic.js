...
// --- START OF FILE game_logic.js ---
// --- DEEL 5      van 8 dit code blok    --- (Focus: Oplossen ReferenceError in aiControlCoop + behoud vorige logica)
...
let aiIsCurrentlyTargetingCaptureBoss = false; // Vlag voor 1P AI Demo
 ...
 function aiControl() { // Voor 1P AI Demo & 1P vs AI Normal (wanneer AI P2 aan de beurt is)
    try {
        const is1PAIDemoMode = !isManualControl && !isPlayerTwoAI;
        const isAIP2NormalMode = isPlayerTwoAI && selectedGameMode === 'normal' && currentPlayer === 2;
        if (isCoopAIDemoActive || (isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP') ) return;
        if (!is1PAIDemoMode && !isAIP2NormalMode) {
            if (aiIsCurrentlyTargetingCaptureBoss && is1PAIDemoMode) aiIsCurrentlyTargetingCaptureBoss = false;
            return;
        }
        ...
        if (currentSmoothedShipXForAI === undefined && activeShipForAI) {
            ...
            if (isAIP2NormalMode || is1PAIDemoMode) {
        ...
        if (mustActivelyEvadeCaptureBeam && captureBeamActive && typeof capturingBossId === 'string') {
            ...
            if (beamBoss) {
                ...
                if (horizontalDistanceToBeamCenter < beamDangerRadius && beamBoss.y < shipTopY + activeShipForAI.height) {
                    ...
                    if (is1PAIDemoMode) aiIsCurrentlyTargetingCaptureBoss = false;
        ...
        if (is1PAIDemoMode && !isDodgingThreat && thisAICanBeCapturedThisTurn && captureBeamActive && capturingBossObject && capturingBossObject.state === 'capturing' && !isShipCapturedForAI && !isWaitingForRespawn) {
            ...
            if (aiIsCurrentlyTargetingCaptureBoss) {
                ...
                // Explicitly set shouldTryShoot to false when moving to capture for 1P AI Demo
                if (isMovingToCapture) {
                    shouldTryShoot = false;
                }
            }
        } else if (is1PAIDemoMode && aiIsCurrentlyTargetingCaptureBoss && (!captureBeamActive || !capturingBossObject || capturingBossObject.state !== 'capturing' || !thisAICanBeCapturedThisTurn || mustActivelyEvadeCaptureBeam)) {
            aiIsCurrentlyTargetingCaptureBoss = false;
            isMovingToCapture = false;
        }


        if (!isMovingToCapture && (!aiIsCurrentlyTargetingCaptureBoss || !is1PAIDemoMode) && !isShowingBlockingMessage && !isDodgingThreat) {
        ...
        if (now < aiStabilizationEndTime && !isShowingBlockingMessage && (!aiIsCurrentlyTargetingCaptureBoss || !is1PAIDemoMode) && !isTargetingThreeSecondRuleBoss && !isDodgingThreat) {
            desiredTargetX = targetCenterShipX;
        } else if (!isShowingBlockingMessage && (!aiIsCurrentlyTargetingCaptureBoss || !is1PAIDemoMode) && !isTargetingThreeSecondRuleBoss && !isDodgingThreat) {
             if (!isChallengingStage && !isInvincibleForAI) {
            ...
            if (isDodgingThreat) {
                ...
                for (const fs of fallingShips) {
                    let isOwnShip = false;
                     if (isAIP2NormalMode && fs.targetPlayerId === String(currentPlayer)) { isOwnShip = true; }
                     else if (is1PAIDemoMode && fs.targetPlayerId === 'player1') { isOwnShip = true; }
                     if (!closestFallingShip && !isOwnShip && fs.targetPlayerId) isOwnShip = true;
                ...
                if (!isMovingToCapture || !is1PAIDemoMode) {
                    targetEnemyForAI = null;
                    ...
                    for (const enemy of enemies) {
                        ...
                        if (is1PAIDemoMode && aiIsCurrentlyTargetingCaptureBoss && enemy.id === capturingBossId) {
                             continue;
                        }
                        ...
                        if (enemy.type === ENEMY3_TYPE && enemy.hasCapturedShip) currentScore += 10000;
                    ...
                    if (targetEnemyForAI) {
                        ...
                        const baseAlignmentFactor = isDualActiveForAI ? 1.35 : 1.0;

                        if (isChallengingStage) {
                            alignmentThresholdForShooting = effectiveShipWidth * 2.5 * baseAlignmentFactor;
                        } else if (targetEnemyForAI.state === 'in_grid' || targetEnemyForAI.state === 'preparing_capture') {
                            alignmentThresholdForShooting = effectiveShipWidth * (GRID_SHOOT_ALIGNMENT_FACTOR * 1.8) * baseAlignmentFactor;
                        } else {
                            alignmentThresholdForShooting = effectiveShipWidth * (FINAL_SHOOT_ALIGNMENT_THRESHOLD * 1.8) * baseAlignmentFactor;
                        }

                        const horizontalDiffToAim = Math.abs(shipCenterX - enemyMidX);
                        let doNotShootThisTarget = false;

                        if (is1PAIDemoMode && aiIsCurrentlyTargetingCaptureBoss && targetEnemyForAI.id === capturingBossId) {
                            doNotShootThisTarget = true;
                        }
                        else if (is1PAIDemoMode &&
                                 targetEnemyForAI.type === ENEMY3_TYPE &&
                                 !targetEnemyForAI.hasCapturedShip &&
                                 (targetEnemyForAI.state === 'following_entrance_path' || targetEnemyForAI.state === 'moving_to_grid')
                                ) {
                            doNotShootThisTarget = true;
                        }
                        else if (is1PAIDemoMode &&
                        ...
                        if (targetEnemyForAI.type === ENEMY3_TYPE && targetEnemyForAI.hasCapturedShip && !doNotShootThisTarget) {
                            shouldTryShoot = true;
        ...
        } else if (is1PAIDemoMode && aiIsCurrentlyTargetingCaptureBoss && isShowingBlockingMessage && !isDodgingThreat) {
            shouldTryShoot = false;
            targetEnemyForAI = null;
        } else if (isTargetingThreeSecondRuleBoss && isShowingBlockingMessage && !isDodgingThreat) {
            shouldTryShoot = false;
        }
        ...
        // Finale overrides voor `shouldTryShoot`
        if (is1PAIDemoMode) {
            if (aiIsCurrentlyTargetingCaptureBoss) { shouldTryShoot = false; }
            if (isMovingToCapture) { shouldTryShoot = false; } // Belangrijkste fix voor dit punt
        }
        if (mustActivelyEvadeCaptureBeam) { shouldTryShoot = false; }
        if (isDodgingThreat) { shouldTryShoot = false; }


        if (shouldTryShoot && !isShowingBlockingMessage) {
        ...
        aiNeedsStabilization = true;
        smoothedShipX = ship ? ship.x : (gameCanvas ? gameCanvas.width / 2 : 0);
        if (aiIsCurrentlyTargetingCaptureBoss && !isManualControl && !isPlayerTwoAI) aiIsCurrentlyTargetingCaptureBoss = false;
    }
 }
...
function aiControlCoop() {
    // --- GEEN WIJZIGINGEN HIER, FOCUS OP 1P AI DEMO ---
    ...
    if (p1CanAct && ship1 && !p1IsHuman) {
        ...
        // LET OP: De gedeconstrueerde variabelen hieronder krijgen een default van false om ReferenceError te voorkomen
        let { desiredTargetX: dt1, shouldTryShoot: sts1, targetEnemyForAI: te1, isLastLifePartnerSave: p1LastLifePartnerSave = false, isThreeSecondRuleTarget: p1ThreeSecRule = false } = calculateAIDesiredState(ship1, smoothedShip1X, player1IsDualShipActive, enemies, enemyBullets, fallingShips, isPlayer1Invincible, isPlayer1ShipCaptured, isPlayer1WaitingForRespawn, now, canvasWidth, p1CompletelyBlocked, 'p1');
        ...
        if (!p1LastLifePartnerSave && !p1ThreeSecRule) {
    ...
    if (p2CanAct && ship2) {
        ...
        // LET OP: De gedeconstrueerde variabelen hieronder krijgen een default van false om ReferenceError te voorkomen
        let { desiredTargetX: dt2, shouldTryShoot: sts2, targetEnemyForAI: te2, isLastLifePartnerSave: p2LastLifePartnerSave = false, isThreeSecondRuleTarget: p2ThreeSecRule = false } = calculateAIDesiredState(ship2, smoothedShip2X, player2IsDualShipActive, enemies, enemyBullets, fallingShips, isPlayer2Invincible, isPlayer2ShipCaptured, isPlayer2WaitingForRespawn, now, canvasWidth, p2CompletelyBlocked, 'ai_p2');
        ...
        if (!p2LastLifePartnerSave && !p2ThreeSecRule) {
...
function calculateAIDesiredState(currentShip, currentSmoothedX, isShipDual, gameEnemies, gameEnemyBullets, gameFallingShips, isShipInvincible, isThisShipCaptured, isThisShipWaitingForRespawn, currentTime, gameCanvasWidth, isCurrentShipCompletelyBlocked, shipIdentifier) {
    // --- GEEN WIJZIGINGEN HIER, FOCUS OP 1P AI DEMO BINNEN aiControl ---
    ...
    if (!isDodgingThreat && !isMovingToCaptureBeam && !isMovingForOwnFallingShip && !isTargetingCapturedPartnerBoss && !isThreeSecondRuleTarget) {
        ...
        for (const enemy of gameEnemies) {
            ...
            // 1P AI Demo (shipIdentifier 'ai') mag niet targeten als het actief naar beam beweegt
            if (shipIdentifier === 'ai' && isMovingToCapture && enemy.id === capturingBossId) {
                continue;
            }
            // CO-OP AI mag niet targeten als het actief naar beam beweegt
            ...
            if (enemy.type === ENEMY3_TYPE && enemy.hasCapturedShip) {
                if (!isBossWithNeutralShipAndAILowLife) { currentScore += 10000; }
            ...
            if (currentScore > bestTargetScore) {
                ...
                const isCurrent1PAIDemoLogic = (shipIdentifier === 'ai' && !isCoopAIDemoActive && !(isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP'));

                if (isCurrent1PAIDemoLogic && aiIsCurrentlyTargetingCaptureBoss && localTargetEnemyForAI.id === capturingBossId) {
                    doNotShootThisTarget = true;
                }
                else if (isCurrent1PAIDemoLogic &&
                         localTargetEnemyForAI.type === ENEMY3_TYPE &&
                         !localTargetEnemyForAI.hasCapturedShip &&
                         (localTargetEnemyForAI.state === 'following_entrance_path' || localTargetEnemyForAI.state === 'moving_to_grid')
                        ) {
                    doNotShootThisTarget = true;
                }
                else if (isCurrent1PAIDemoLogic &&
                         ...
                         (localTargetEnemyForAI.state === 'in_grid' ||
                          ...
                          (localTargetEnemyForAI.state === 'capturing' && localTargetEnemyForAI.id === capturingBossId)
                         )
                        ) {
                    doNotShootThisTarget = true;
                }
                else if (localTargetEnemyForAI.type === ENEMY3_TYPE && !localTargetEnemyForAI.hasCapturedShip && !isFullGridWave) {
                    if (canThisAIShipBeCaptured && !mustActivelyEvadeCaptureBeam && !captureAttemptMadeThisLevel &&
                        (localTargetEnemyForAI.state === 'in_grid' || localTargetEnemyForAI.state === 'preparing_capture' || localTargetEnemyForAI.state === 'diving_to_capture_position' || localTargetEnemyForAI.state === 'capturing')
                       ) {
                    ...
                    if (isShipDual && !isCurrent1PAIDemoLogic) {
                        ...
                        if (partnerIsActiveAndNeedsDual && localTargetEnemyForAI.type === ENEMY3_TYPE && !localTargetEnemyForAI.hasCapturedShip) { doNotShootThisTarget = true;}
                    }
                }
                ...
                } else if (doNotShootThisTarget) {
                    localShouldTryShoot = false;
                }
                else if (localTargetEnemyForAI.type === ENEMY3_TYPE && localTargetEnemyForAI.hasCapturedShip) {
                    localShouldTryShoot = true;
                }
                else if (Math.abs(shipCenterX - (localTargetEnemyForAI.x + localTargetEnemyForAI.width / 2)) < (effectiveShipWidth * (isShipDual ? 1.1 : 0.9)) ) {
                    localShouldTryShoot = true;
                } else {
                    localShouldTryShoot = false;
                }
    ...
    const isCurrently1PAIDemoForFinalOverride = (shipIdentifier === 'ai' && !isCoopAIDemoActive && !(isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP'));
    if (isCurrently1PAIDemoForFinalOverride) {
        // Deze overrides gebeuren nu aan het einde van aiControl zelf.
    } else { // COOP AI
        if (isMovingToCaptureBeam && aiPlayerActivelySeekingCaptureById === shipIdentifier) { shouldTryShoot = false; }
    }

    if (isDodgingThreat) { shouldTryShoot = false; if(isDodgingThreat) targetEnemyForAI = null; }
