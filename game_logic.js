...
// --- DEEL 1      van 8 dit code blok    --- (Focus: resetWaveInternal - reset AI capture intentie)
...
function resetGameInternal() {
    ...
    // <<< NIEUW: Reset AI capture intentie bij game reset >>>
    aiIntendsToBeCapturedByBossId = null;
    aiIntentionToCaptureStartTime = 0;
    // <<< EINDE NIEUW >>>
...
function resetWaveInternal() {
    ...
    // <<< NIEUW: Reset AI capture intentie bij wave reset >>>
    aiIntendsToBeCapturedByBossId = null;
    aiIntentionToCaptureStartTime = 0;
    // <<< EINDE NIEUW >>>
...
// --- START OF FILE game_logic.js ---
// --- DEEL 4      van 8 dit code blok    --- (Focus: handleEnemyHit - reset AI capture intentie)
...
function handleEnemyHit(enemy, shootingPlayerId = null) {
    ...
    if (enemy.health <= 0) {
        destroyed = true;
        // <<< NIEUW: Reset AI capture intentie als de beoogde baas vernietigd is >>>
        if (aiIntendsToBeCapturedByBossId === enemy.id) {
            aiIntendsToBeCapturedByBossId = null;
            aiIntentionToCaptureStartTime = 0;
        }
        // <<< EINDE NIEUW >>>
...
// --- START OF FILE game_logic.js ---
// --- DEEL 5      van 8 dit code blok    --- (Focus: AI Capture Behavior & CO-OP AI Entrance Dodge)
...
function firePlayerBullet(shooterId = null, isTapEvent = false) {
    ...
    if (isTwoPlayerMode && selectedGameMode === 'coop') {
        if (shooterId === 'player1' && ship1 && player1Lives > 0 && !isPlayer1ShipCaptured && !isPlayer1WaitingForRespawn && !player1NeedsRespawnAfterCapture) { // <<< GEWIJZIGD: !isPlayer1WaitingForRespawn toegevoegd >>>
        ...
        } else if (shooterId === 'player2' && ship2 && player2Lives > 0 && !isPlayer2ShipCaptured && !isPlayer2WaitingForRespawn && !player2NeedsRespawnAfterCapture) { // <<< GEWIJZIGD: !isPlayer2WaitingForRespawn toegevoegd >>>
        ...
        if (ship && playerLives > 0 && !isShipCaptured && !isWaitingForRespawn ) { // <<< GEWIJZIGD: !isWaitingForRespawn toegevoegd >>>
    ...
    if (shooterId === null && !isManualControl && !isCoopAIDemoActive && ship && playerLives > 0 && !isShipCaptured && !isShowingCaptureMessage && !isWaitingForRespawn) { // <<< GEWIJZIGD: !isWaitingForRespawn toegevoegd >>>
...
function handlePlayerInput() {
     try {
        ...
        if (isTwoPlayerMode && selectedGameMode === 'coop') {
            ...
            if (!p1MovementFromTouch) {
                const activePlayerCanControl = ship && playerLives > 0 && !isShipCaptured && !isWaitingForRespawn && !isShowingPlayerGameOverMessage; // <<< GEWIJZIGD: !isWaitingForRespawn toegevoegd >>>
        ...
        if (isTwoPlayerMode && selectedGameMode === 'coop') {
            ...
            const activePlayerCanShoot = ship && playerLives > 0 && !isShipCaptured && !isWaitingForRespawn && !isShowingPlayerGameOverMessage && !isShowingIntro && !inCoopLevel1IntroStrict; // <<< GEWIJZIGD: !isWaitingForRespawn toegevoegd >>>
...
// --- AI control functies (aiControl, aiControlCoop, fireCoopAIBullet, calculateAIDesiredState) ---
 ...
 function aiControl() {
    try {
        ...
        let aiCanBeCaptured = false;

        if (isPlayerTwoAI && selectedGameMode === 'normal' && currentPlayer === 2) {
            if (!ship || playerLives <= 0 || isShipCaptured || isWaitingForRespawn || isShowingPlayerGameOverMessage) { // <<< GEWIJZIGD: isWaitingForRespawn toegevoegd >>>
            ...
            aiCanBeCaptured = playerLives > 1 && !isDualShipActive && !isWaitingForRespawn; // <<< GEWIJZIGD: !isWaitingForRespawn toegevoegd >>>
        } else if (!isManualControl && !isPlayerTwoAI) { // 1P AI Demo
            if (playerLives <= 0 || !ship || !gameCanvas || !isInGameState || gameOverSequenceStartTime > 0 || isWaitingForRespawn || isShowingPlayerGameOverMessage || isShipCaptured || isShowingCaptureMessage) { // <<< GEWIJZIGD: isWaitingForRespawn toegevoegd >>>
                if (ship) ship.targetX = ship.x;
                aiNeedsStabilization = true;
                smoothedShipX = ship ? ship.x : (gameCanvas ? gameCanvas.width / 2 : 0);
                return;
        ...
        if (now < aiStabilizationEndTime && !isShowingBlockingMessage) {
            ...
            if (isDodgingThreat) {
                ...
                isMovingToCapture = false;
                 if (aiCanBeCaptured && captureBeamActive && capturingBossId && !isShipCapturedForAI && !isWaitingForRespawn) {
                    ...
                    if (capturingBoss) {
                        ...
                        if (dxToBeam < (effectiveShipWidth * 0.8) && (now - (capturingBoss.captureStartTime || 0) >= AI_CAPTURE_BEAM_APPROACH_DELAY_MS)) {
                            ...
                            shouldTryShoot = false; // <<< AI DEMO: Niet schieten als het actief naar de beam beweegt >>>
                            targetEnemyForAI = null;
                ...
                if (!isMovingToCapture) {
                    ...
                    if (targetEnemyForAI) {
                        ...
                        if (horizontalDiffToAim < alignmentThresholdForShooting) {
                            ...
                            // <<< AI DEMO: Logica voor niet schieten als de AI van plan is gevangen te worden >>>
                            if (aiIntendsToBeCapturedByBossId === targetEnemyForAI.id &&
                                now < aiIntentionToCaptureStartTime + AI_CAPTURE_INTENTION_DURATION) {
                                blockShootingThisTarget = true;
                            } else if (aiCanBeCaptured && targetEnemyForAI.type === ENEMY3_TYPE &&
                                ...
                                // Dit is de initiële intentie, nog niet het actieve wachten
                                // De daadwerkelijke vlag wordt in calculateAIDesiredState gezet.
                                // Hier controleren we of dit de baas IS waar we op wachten
                                if (aiIntendsToBeCapturedByBossId === targetEnemyForAI.id) {
                                     blockShootingThisTarget = true;
                                }
                                // Als er geen intentie is, maar deze baas is een goede kandidaat,
                                // dan kan de AI overwegen de intentie te zetten in calculateAIDesiredState.
                                // Hier schiet het voor nu wel, tenzij de intentie al bestaat.
                            }
                            // <<< EINDE AI DEMO LOGICA >>>
...
function calculateAIDesiredState(currentShip, currentSmoothedX, isShipDual, gameEnemies, gameEnemyBullets, gameFallingShips, isShipInvincible, isThisShipCaptured, isThisShipWaitingForRespawn, currentTime, gameCanvasWidth, isCurrentShipCompletelyBlocked, shipIdentifier) {
    ...
    const canThisAIShipBeCaptured = livesOfThisAIShip > 1 && !isShipDual && !isThisShipWaitingForRespawn;
    ...
    if (!isShipInvincible) {
        let threateningBullets = [];
        // <<< GEWIJZIGD: Aangepaste bulletLookahead en bulletBuffer voor COOP AI tijdens entree >>>
        let bulletLookahead, bulletBuffer;
        if (isEntrancePhaseActive && (isCoopAIDemoActive || (isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP'))) {
            bulletLookahead = ENTRANCE_BULLET_DODGE_LOOKAHEAD * 2.5; // Aanzienlijk groter voor COOP entree
            bulletBuffer = ENTRANCE_BULLET_DODGE_BUFFER * 2.0;    // Bredere buffer voor COOP entree
        } else {
            bulletLookahead = FINAL_DODGE_LOOKAHEAD * ((isCoopAIDemoActive || (isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP')) ? 1.6 : 0.8);
            bulletBuffer = FINAL_DODGE_BUFFER_BASE * ((isCoopAIDemoActive || (isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP')) ? 1.7 : 0.8);
        }
        // <<< EINDE GEWIJZIGD >>>
        ...
        if (threateningBullets.length > 0) {
            ...
            // <<< GEWIJZIGD: Aangepaste dodgeAmountBase voor COOP AI tijdens entree >>>
            let dodgeAmountBase;
            if (isEntrancePhaseActive && (isCoopAIDemoActive || (isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP'))) {
                dodgeAmountBase = effectiveShipWidth * 3.5; // Grotere dodge voor COOP entree
            } else {
                dodgeAmountBase = effectiveShipWidth * ((isCoopAIDemoActive || (isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP')) ? 3.0 : 2.0);
            }
            // <<< EINDE GEWIJZIGD >>>
        ...
        for (const enemy of gameEnemies) {
            ...
            if (currentScore > bestTargetScore) {
                ...
                if (localTargetEnemyForAI.type === ENEMY3_TYPE && !localTargetEnemyForAI.hasCapturedShip && !isFullGridWave) {
                    if (aiIntendsToBeCapturedByBossId === localTargetEnemyForAI.id &&
                        currentTime < aiIntentionToCaptureStartTime + AI_CAPTURE_INTENTION_DURATION) {
                        doNotShootThisBossSpecifically = true;
                    } else if (canThisAIShipBeCaptured && !captureAttemptMadeThisLevel &&
                        ...
                        // Als de AI nog geen intentie heeft, en deze baas is een kandidaat,
                        // registreer de intentie nu.
                        if (aiIntendsToBeCapturedByBossId === null) {
                             aiIntendsToBeCapturedByBossId = localTargetEnemyForAI.id;
                             aiIntentionToCaptureStartTime = currentTime;
                             doNotShootThisBossSpecifically = true; // Niet schieten tijdens de intentieperiode
                        } else if (aiIntendsToBeCapturedByBossId === localTargetEnemyForAI.id) {
                             // De intentie bestaat al voor deze baas, dus niet schieten.
                             doNotShootThisBossSpecifically = true;
                        }
                    }
                }
...
// --- START OF FILE game_logic.js ---
// --- DEEL 7      van 8 dit code blok    --- (Focus: moveEntities - reset AI capture intentie in 'capturing' state)
    ...
    try {
        ...
        if (gameCanvas && !isTouchActiveGame) { // Alleen keyboard/gamepad beweging als touch NIET actief is.
            if (isManualControl) {
                if (isTwoPlayerMode && selectedGameMode === 'coop') {
                    ...
                    if (ship && playerLives > 0 && !isShipCaptured && !isWaitingForRespawn && !isShowingPlayerGameOverMessage && gameOverSequenceStartTime === 0) { // <<< GEWIJZIGD: !isWaitingForRespawn toegevoegd >>>
        ...
        if (!isManualControl ||
            ...
            if (isCoopAIDemoActive || (isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP')) {
                ...
                if (playerLives > 0 && !isShipCaptured && !isWaitingForRespawn && !isShowingPlayerGameOverMessage && gameOverSequenceStartTime === 0) { // <<< GEWIJZIGD: !isWaitingForRespawn toegevoegd >>>
                ...
                if (fs.y >= TARGET_FALLING_SHIP_Y) {
                    ...
                    if (rescuerIsActiveAndExists && !rescuerAlreadyDual) {
                        ...
                        if (playerShipIsCurrentlyCaptured && !(isCoopAIDemoActive || (isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP'))) {
                            ...
                            else if (ship) { isInvincible = true; invincibilityEndTime = now + INVINCIBILITY_DURATION_MS; } // <<< GEWIJZIGD: check 'ship' direct
                        }
        ...
        for (let i = enemies.length - 1; i >= 0; i--) {
                 ...
                 case 'capturing': {
                    ...
                    if (captureBeamActive && captureBeamProgress >= 0.95) {
                        ...
                        if (boxHeight > 0) { const beamBoundingBox = { x: boxX, y: boxY, width: boxWidth, height: boxHeight }; let shipHitObject = null; let playerHitId = 0;
                            ...
                            if (shipHitObject) {
                                enemy.capturedShipDimensions = { width: shipHitObject.width, height: shipHitObject.height }; enemy.hasCapturedShip = true; stopSound('captureSound'); enemy.state = 'showing_capture_message'; enemy.targetGridX = null; enemy.targetGridY = null; enemy.initialCaptureAnimationY = shipHitObject.y; enemy.captureAnimationRotation = 0; isShowingCaptureMessage = true; captureMessageStartTime = now; capturedBossIdWithMessage = enemy.id; playSound('shipCapturedSound', false, 0.3); csCurrentChainHits = 0; csCurrentChainScore = 0; csLastHitTime = 0; csLastChainHitPosition = null; normalWaveCurrentChainHits = 0; normalWaveCurrentChainScore = 0; normalWaveLastHitTime = 0; normalWaveLastHitPosition = null;
                                // <<< NIEUW: Reset AI capture intentie als het schip gevangen is >>>
                                if (aiIntendsToBeCapturedByBossId === enemy.id) {
                                    aiIntendsToBeCapturedByBossId = null;
                                    aiIntentionToCaptureStartTime = 0;
                                }
                                // <<< EINDE NIEUW >>>
                                handlePlayerShipCollision(playerHitId, false, now, true); if((isCoopAIDemoActive || (isPlayerTwoAI && selectedOnePlayerGameVariant === '1P_VS_AI_COOP')) && aiPlayerActivelySeekingCaptureById) aiPlayerActivelySeekingCaptureById = null; break;
                            }
                        }
                    } break;
                 }
            ...
            if (enemy && !isShowingPlayerGameOverMessage && !(isTwoPlayerMode && selectedGameMode === 'coop' && (isPlayer1ShowingGameOverMessage || isPlayer2ShowingGameOverMessage)) && gameOverSequenceStartTime === 0) {
                ...
                if (collisionStates.includes(enemy.state)) {
                    ...
                    if (isTwoPlayerMode && selectedGameMode === 'coop') {
                        if (ship1 && player1Lives > 0 && !isPlayer1ShipCaptured && !isPlayer1Invincible && !isPlayer1WaitingForRespawn && !isPlayer1ShowingGameOverMessage && !player1NeedsRespawnAfterCapture) shipsToCollideWith.push({shipObj: ship1, playerNum: 1, dual: player1IsDualShipActive}); // <<< GEWIJZIGD: !isPlayer1WaitingForRespawn toegevoegd >>>
                        if (ship2 && player2Lives > 0 && !isPlayer2ShipCaptured && !isPlayer2Invincible && !isPlayer2WaitingForRespawn && !isPlayer2ShowingGameOverMessage && !player2NeedsRespawnAfterCapture) shipsToCollideWith.push({shipObj: ship2, playerNum: 2, dual: player2IsDualShipActive}); // <<< GEWIJZIGD: !isPlayer2WaitingForRespawn toegevoegd >>>
                    } else { // 1P Classic, 1P_VS_AI_NORMAL, 2P_NORMAL
                        if (ship && playerLives > 0 && !isShipCaptured && !isShowingPlayerGameOverMessage && !isInvincible && !isWaitingForRespawn) { // <<< GEWIJZIGD: !isWaitingForRespawn toegevoegd >>>
        ...
        if (!isShowingPlayerGameOverMessage && !(isTwoPlayerMode && selectedGameMode === 'coop' && (isPlayer1ShowingGameOverMessage || isPlayer2ShowingGameOverMessage)) && gameOverSequenceStartTime === 0) {
            let shipsToBulletHitCheck = [];
             if (isTwoPlayerMode && selectedGameMode === 'coop') {
                if (ship1 && player1Lives > 0 && !isPlayer1ShipCaptured && !isPlayer1Invincible && !isPlayer1WaitingForRespawn && !isPlayer1ShowingGameOverMessage && !player1NeedsRespawnAfterCapture) shipsToBulletHitCheck.push({shipObj: ship1, playerNum: 1, dual: player1IsDualShipActive}); // <<< GEWIJZIGD: !isPlayer1WaitingForRespawn toegevoegd >>>
                if (ship2 && player2Lives > 0 && !isPlayer2ShipCaptured && !isPlayer2Invincible && !isPlayer2WaitingForRespawn && !isPlayer2ShowingGameOverMessage && !player2NeedsRespawnAfterCapture) shipsToBulletHitCheck.push({shipObj: ship2, playerNum: 2, dual: player2IsDualShipActive}); // <<< GEWIJZIGD: !isPlayer2WaitingForRespawn toegevoegd >>>
            } else { // 1P Classic, 1P_VS_AI_NORMAL, 2P_NORMAL
                if (ship && playerLives > 0 && !isShipCaptured && !isShowingPlayerGameOverMessage && !isInvincible && !isWaitingForRespawn) { // <<< GEWIJZIGD: !isWaitingForRespawn toegevoegd >>>
...
function triggerGridFiring() {
    ...
    if (isTwoPlayerMode && selectedGameMode === 'coop') {
        if ((player1Lives > 0 && ship1 && !isPlayer1ShipCaptured && !isPlayer1WaitingForRespawn && !isPlayer1ShowingGameOverMessage && !player1NeedsRespawnAfterCapture) || // <<< GEWIJZIGD: !isPlayer1WaitingForRespawn toegevoegd >>>
            (player2Lives > 0 && ship2 && !isPlayer2ShipCaptured && !isPlayer2WaitingForRespawn && !isPlayer2ShowingGameOverMessage && !player2NeedsRespawnAfterCapture)) { // <<< GEWIJZIGD: !isPlayer2WaitingForRespawn toegevoegd >>>
            canFireBasedOnPlayerState = true;
        }
    } else { // 1P Classic, 1P_VS_AI_NORMAL, 2P_NORMAL
        if (playerLives > 0 && ship && !isShipCaptured && !isWaitingForRespawn && !isShowingPlayerGameOverMessage) { // <<< GEWIJZIGD: !isWaitingForRespawn toegevoegd >>>
            canFireBasedOnPlayerState = true;
        }
    }
...
// --- START OF FILE game_logic.js ---
// --- DEEL 8      van 8 dit code blok    --- (Focus: Consistentie met !isWaitingForRespawn in condities)
...
function runSingleGameUpdate(timestamp) {
    try {
        ...
        if (isTwoPlayerMode && selectedGameMode === 'coop') {
            ...
            activePlayersForLogic = playerLives > 0 && !isShipCaptured && !isWaitingForRespawn && !isShowingPlayerGameOverMessage;
        }
        ...
        if(isManualControl && connectedGamepadIndex !== null){
            if(isTwoPlayerMode && selectedGameMode === 'coop'){ // Geldt voor Human COOP en 1P vs AI COOP (P1 is mens)
                if(player1Lives > 0 && !isPlayer1ShipCaptured && !isPlayer1WaitingForRespawn && !isPlayer1ShowingGameOverMessage && !player1NeedsRespawnAfterCapture) primaryControllerCanExit = true;
            } else { // 1P Classic, 1P_VS_AI_NORMAL (P1's beurt), 2P_NORMAL (actieve speler)
                if(playerLives > 0 && !isShipCaptured && !isWaitingForRespawn && !isShowingPlayerGameOverMessage && (!isPlayerTwoAI || (isPlayerTwoAI && currentPlayer === 1))) primaryControllerCanExit = true;
            }
        }
        ...
        if(isManualControl && isTwoPlayerMode && selectedGameMode === 'coop' && !isPlayerTwoAI && connectedGamepadIndexP2 !== null){
            if(player2Lives > 0 && !isPlayer2ShipCaptured && !isPlayer2WaitingForRespawn && !isPlayer2ShowingGameOverMessage && !player2NeedsRespawnAfterCapture) secondaryControllerCanExit = true;
             ...
             if(playerLives > 0 && !isShipCaptured && !isWaitingForRespawn && !isShowingPlayerGameOverMessage && currentPlayer === 2) secondaryControllerCanExit = true;
        }
                ...
                const shipBaseY = gameCanvas ? gameCanvas.height - SHIP_HEIGHT - SHIP_BOTTOM_MARGIN : 500;
                 if (isTwoPlayerMode && selectedGameMode === 'coop') {
                    if (player1NeedsRespawnAfterCapture && player1Lives > 0) {
                        ...
                        isPlayer1ShipCaptured = false;
                        isPlayer1Invincible = true; player1InvincibilityEndTime = now + INVINCIBILITY_DURATION_MS;
                        isPlayer1WaitingForRespawn = false; // Wordt false na respawn
                        player1NeedsRespawnAfterCapture = false;
                        player1CaptureRespawnX = 0;
                    ...
                    if (player2NeedsRespawnAfterCapture && player2Lives > 0) {
                        ...
                        isPlayer2ShipCaptured = false;
                        isPlayer2Invincible = true; player2InvincibilityEndTime = now + INVINCIBILITY_DURATION_MS;
                        isPlayer2WaitingForRespawn = false; // Wordt false na respawn
                        player2NeedsRespawnAfterCapture = false;
                        player2CaptureRespawnX = 0;
                    ...
                    if (isShipCaptured && playerLives > 0) {
                        ...
                        isShipCaptured = false;
                        isInvincible = true; invincibilityEndTime = now + INVINCIBILITY_DURATION_MS;
                        isWaitingForRespawn = false; // Wordt false na respawn
                        aiNeedsStabilization = true;
                    ...
                    if (isShipCaptured && playerLives > 0) {
                        ...
                        isInvincible = true; invincibilityEndTime = now + INVINCIBILITY_DURATION_MS;
                        isWaitingForRespawn = true; // Voor handmatige spellen, start de respawn timer nu.
                        respawnTime = now + RESPAWN_DELAY_MS; // <<< TOEGEVOEGD: Start respawn timer hier >>>
        ...
        if (inNormalIntro && noPlayerGameOverIsActive && gameOverSequenceStartTime === 0) {
            ...
            if (introStep === 1) { // PLAYER X
                ...
                if (!stageIntroSoundPlayed) {
                     ...
                     else if (isTwoPlayerMode && selectedGameMode === 'coop' && !isPlayerTwoAI && !isCoopAIDemoActive && level === 1) {
                        // Voor Human 2-Player CO-OP LEVEL 1: speel levelUpSound.
                        // De 'startSound' is al gespeeld in baseStartGame.
                        playSound('levelUpSound', false, 0.2);
                        stageIntroSoundPlayed = true;
                     }
                     else if (isTwoPlayerMode && selectedGameMode === 'coop' && !isPlayerTwoAI && !isCoopAIDemoActive && level > 1) {
                         // Voor Human 2P CO-OP level > 1, ook levelUpSound
                         playSound('levelUpSound', false, 0.2);
                         stageIntroSoundPlayed = true;
                     }
        ...
        if (noSpecialOrNormalIntroRunning && isInGameState && noPlayerGameOverIsActive && gameOverSequenceStartTime === 0) {
            if (!isShowingCSBonusScreen || (isShowingCSBonusScreen && isManualControl)) {
                ...
                if (typeof moveEntities === 'function') moveEntities(); // <<< LET OP: `moveEntities` wordt hier aangeroepen >>>
                updateExplosions();
