...
// --- START OF FILE setup_utils.js ---
// --- DEEL 3      van 3 dit code blok    --- (Relevant deel aangepast)


// ... (bestaande code in setup_utils.js deel 1 & 2) ...
...
function handleTouchEndGlobal(event) {
    ...
    if (isTouchActiveGame && isInGameState) {
        if (isTap) {
            ...
            if (typeof MARGIN_SIDE !== 'undefined' && typeof MARGIN_TOP !== 'undefined' &&
                typeof SCORE_OFFSET_Y !== 'undefined' && gameCanvas && gameCtx) {

                const UI_TAP_CHECK_FONT = "20px 'Press Start 2P'";
                gameCtx.font = UI_TAP_CHECK_FONT;

                let label2PText = "2UP";
                if (isInGameState) {
                    if (typeof isCoopAIDemoActive !== 'undefined' && isCoopAIDemoActive) {
                        label2PText = "DEMO-2";
                    } else if (typeof isPlayerTwoAI !== 'undefined' && isPlayerTwoAI) {
                        if (typeof selectedGameMode !== 'undefined' && selectedGameMode === 'coop' &&
                            typeof selectedOnePlayerGameVariant !== 'undefined' && selectedOnePlayerGameVariant === '1P_VS_AI_COOP') {
                            label2PText = "AI P2";
                        } else if (typeof selectedGameMode !== 'undefined' && selectedGameMode === 'normal') {
                            label2PText = "AI P2";
                        }
                    }
                }
                ...
                const effectivelyTwoPlayerUIForTap = (typeof isTwoPlayerMode !== 'undefined' && isTwoPlayerMode) ||
                                                     (typeof isPlayerTwoAI !== 'undefined' && isPlayerTwoAI && typeof selectedGameMode !== 'undefined' && selectedGameMode === 'coop');

                if (effectivelyTwoPlayerUIForTap && isInGameState) {
                    if (selectedGameMode === 'coop') {
                         if (typeof player2Score !== 'undefined') currentScore2PValue = player2Score;
                    } else if (selectedGameMode === 'normal') {
                        if (typeof isPlayerTwoAI !== 'undefined' && isPlayerTwoAI && typeof currentPlayer !== 'undefined' && currentPlayer === 2 && typeof score !== 'undefined') {
                            currentScore2PValue = score;
                        } else if (typeof player2Score !== 'undefined') {
                ...
                const areaTextWidth = Math.max(label2PWidth, score2PWidth);
                const areaActualWidth = areaTextWidth + 2 * SCORE_AREA_TAP_MARGIN;
                const areaEndX = gameCanvas.width - MARGIN_SIDE + SCORE_AREA_TAP_MARGIN;
                const areaStartX = areaEndX - areaActualWidth;

                let approxFontHeight = 20;
                try {
                    const fontHeightMatch = UI_TAP_CHECK_FONT.match(/(\d+)px/);
                    if (fontHeightMatch && fontHeightMatch[1]) {
                        approxFontHeight = parseInt(fontHeightMatch[1], 10);
                    }
                } catch (fe) { /* use default 20px */ }

                const areaStartY = MARGIN_TOP - SCORE_AREA_TAP_MARGIN;
                const areaEndY = MARGIN_TOP + SCORE_OFFSET_Y + 5 + approxFontHeight + SCORE_AREA_TAP_MARGIN;

                if (canvasTapX >= areaStartX && canvasTapX <= areaEndX &&
                    canvasTapY >= areaStartY && canvasTapY <= areaEndY) {
                    tapped2UpArea = true;
                }
            }
            ...
            if (tapped2UpArea) {
                if (lastTapArea === '2up' && (now - lastTapTimestamp < DOUBLE_TAP_MAX_INTERVAL)) {
                    if (typeof stopGameAndShowMenu === 'function') {
                        stopGameAndShowMenu(); // This will reset isInGameState and demo flags
                        lastTapArea = null;
                        lastTapTimestamp = 0;
                        isTouchActiveGame = false;
                        p1FireInputWasDown = false; p2FireInputWasDown = false; // Reset fire inputs
            ...
            if (selectedFiringMode === 'single' && !tapped2UpArea) { // Alleen als NIET 2UP double tap was
                if (now - lastTapTime > SHOOT_COOLDOWN / 2) {
                    ...
                    // Tijdelijk pXFireInputWasDown zetten voor firePlayerBullet
                    const originalP1FireInput = p1FireInputWasDown;
                    const originalP2FireInput = p2FireInputWasDown;
                    ...
                    // Herstel pXFireInputWasDown direct na de call
                    p1FireInputWasDown = originalP1FireInput;
                    p2FireInputWasDown = originalP2FireInput;

                    lastTapTime = now;
                }
            }
        }
        // Bestaande resets van shootPressed etc. voor touch
        shootPressed = false;
...
// <<< EINDE WIJZIGING in handleTouchEndGlobal >>>


// --- Keyboard Event Handlers ---
function handleKeyDown(e) { /* ... ongewijzigd ... */ }
function handleKeyUp(e) { /* ... ongewijzigd ... */ }


// --- Gamepad Event Handlers ---
function handleGamepadConnected(event) { /* ... ongewijzigd ... */ }
function handleGamepadDisconnected(event) { /* ... ongewijzigd ... */ }

// --- High Score ---
function saveHighScore() { /* ... ongewijzigd ... */ }
function loadHighScore() { /* ... ongewijzigd ... */ }

// --- Pauze Functies ---
function pauseAllSounds() { /* ... ongewijzigd ... */ }
function resumeAllSounds() { /* ... ongewijzigd ... */ }
function togglePause() { /* ... ongewijzigd ... */ }


function processSingleController(gamepad, previousButtonStates) { /* ... ongewijzigd ... */ }
function triggerFinalGameOverSequence() { /* ... ongewijzigd ... */ }
